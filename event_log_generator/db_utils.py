...
"""
def read_events_into_df(db_connection,start_date = None, end_date =None, resource_id = None):
    if db_connection is None:
        raise ValueError('db_connection must be set')
    print('Reading events from database', start_date, end_date)
    if start_date is None or end_date is None:
        df = pd.read_sql('SELECT EVENT,CASE_ID,ACTIVITY_NAME, TIME_OF_EVENT, LIFECYCLE_PHASE, RESOURCE, RESOURCE_TYPE, REMARKS FROM LAS2PEERMON.MESSAGE WHERE CASE_ID IS NOT NULL AND RESOURCE = %s', con=db_connection, params=(resource_id,))
    else:
        statement = 'SELECT EVENT,CASE_ID,ACTIVITY_NAME, TIME_OF_EVENT, LIFECYCLE_PHASE, RESOURCE, RESOURCE_TYPE, REMARKS FROM LAS2PEERMON.MESSAGE WHERE CASE_ID IS NOT NULL AND RESOURCE = %s AND TIME_STAMP BETWEEN %s AND %s'
        # format the statement
        df = pd.read_sql(statement, con=db_connection, params=(resource_id,start_date, end_date))
